...
- **Tray / survive window close:** `electron/tray.ts` (`TrayController`) owns an optional system-tray icon, present **only while `scheduler.armedCount > 0`** (≥1 enabled scheduled workflow). `refresh()` is idempotent — it creates the tray when the count rises above zero, tears it down when it returns to zero, and otherwise refreshes the tooltip + menu ("Open AIchemist" / count label / "Quit AIchemist"). `main.ts` wires `scheduler.onJobsChanged(() => tray.refresh())`. The non-darwin `window-all-closed` handler no longer quits unconditionally: it keeps the app alive (tray-only, scheduler still firing) when `armedCount > 0` and not already quitting; otherwise it quits as before. An `isQuitting` flag (set by the tray "Quit" action and `before-quit`) forces a real quit through. The tray icon is an embedded base64 PNG (no main-process asset pipeline). Firing while the app is *fully closed* (OS-level launchd/Task Scheduler) remains out of scope.
