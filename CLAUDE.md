...
- **No `runGatedTool` / approval gate.** Codex executes tools itself; we do NOT route them through the shared gate. Instead `resolveSandboxPolicy()` maps the turn flags onto Codex's `sandboxMode` (`read-only` / `workspace-write` / `danger-full-access`) and `approvalPolicy` (`never` / `on-request` / `on-failure` / `untrusted`): `noTools` → read-only + never; `nonInteractive` (autonomous workflow) → workspace-write + never; interactive default → workspace-write + on-failure. **Surfacing Codex's interactive `on-request` approvals through AIchemist's approval UI is not yet implemented** (the `codex exec` transport can't do interactive callbacks — needs the app-server JSON-RPC protocol; tracked by #128/#127).
- **Thread model:** `Codex.startThread(opts)` / `resumeThread(id, opts)` then `thread.runStreamed(input)`. The thread id is persisted via `providerSessionStore` under the `codex` slice (`{ threadId }`); resume reuses it. Model / sandbox / approval / `workingDirectory` are set at **thread creation** (`ThreadOptions`), not per-turn.
