...
- **Load-older pagination:** `startReached` (Virtuoso fires it when the user scrolls near index 0) is wired to `useLoadOlderMessages()` (`src/lib/hooks/useLoadOlderMessages.ts`), which fetches the next-older page via `ipc.getSession(sessionId, { limit, beforeMessageId })` and prepends it through the store's `prependMessages()` action — see "Session history hydration" below for the full paginated flow. Virtuoso needs `firstItemIndex` decremented by exactly the number of newly-prepended messages to anchor the viewport instead of jumping (the classic "Prepend Items" pattern) — `TimelinePanel` tracks it in a `firstItemIndex` state initialized to an arbitrary large constant (`FIRST_ITEM_INDEX_START`) and resets it on session switch alongside the other scroll-reset state. A per-session in-flight guard in the hook (a ref, not Zustand) drops overlapping fetches if `startReached` fires again before the previous page lands.
...
**Session history hydration (paginated — issue #208 follow-up):** `listSessions()` returns metadata only (`messages: []`). When `activeSessionId` changes, `useSessionHydration` (mounted in `App.tsx`) calls `ipc.getSession(sessionId, { limit: INITIAL_MESSAGE_PAGE_SIZE })` — the most recent 150 messages, not the full history — and calls `hydrateSession()` on the store. `mergeSessions()` deliberately preserves existing messages to avoid a race where a metadata refresh wipes hydrated history.

- **`getSession()` / `GET_SESSION`** (`electron/sessions.ts`) is backwards compatible: called with no options (or by any caller that hasn't opted in) it still returns the full message + tool-call history in one IPC call, exactly as before. Passing `{ limit }` and/or `{ limit, beforeMessageId }` opts into pagination instead — `beforeMessageId` cursors on the given message's `rowid` (insertion order, not `created_at`, since ISO-string ties are possible within the same millisecond) and returns the page immediately older than it, oldest-first like the full-history result. The returned `Session.has_more_messages` flag says whether an older page remains; it's always `false` for the non-paginated (full-history) call since nothing is left to page through. A stale `beforeMessageId` (message no longer exists) returns an empty page rather than silently re-fetching the newest one.
- **Loading older messages:** `TimelinePanel`'s Virtuoso list calls `useLoadOlderMessages()` from `startReached` — see "Timeline virtualization" above for the Virtuoso-side wiring (`firstItemIndex`, the in-flight guard). The fetched page is merged in via the store's `prependMessages(sessionId, messages, hasMore)` action, which de-dupes against already-loaded messages and updates `sessionHasMoreMessages[sessionId]` (a store field separate from the `sessions` map, so `mergeSessions()`'s metadata-refresh path — which never touches it — can't clobber it).
- **Page sizes are duplicated, not shared:** `DEFAULT_MESSAGE_PAGE_SIZE` (`electron/sessions.ts`), `INITIAL_MESSAGE_PAGE_SIZE` (`useSessionHydration.ts`), and `OLDER_MESSAGES_PAGE_SIZE` (`useLoadOlderMessages.ts`) are separate constants that happen to share a value (150) today — the renderer can only type-import from `electron/`, never value-import the Node-only module (same rule as the tool-round-cap bounds in `SettingsView.tsx`). Keep them in sync by hand if you change the default.
...
- **Turn loop:** streaming `chat()` with an in-process tool-calling loop capped at `readMaxToolRounds()` rounds (`AICHEMIST_MAX_TOOL_ROUNDS` setting, default 8 — see "Configurable tool-round cap" below). Tools are implemented locally (`tool-impls.ts`: write_file, delete_file, execute_bash, web_fetch, plus read_file/list_directory/glob) and approval-gated through the same `requiresApproval()` / `requestApproval()` path as the other providers. `ask_user` is not in `tool-impls.ts` — it is registered per-provider and plumbed through `electron/agent/question.ts` (`requestQuestion` / `resolveQuestion`). Managed MCP servers are reachable through `createManagedMcpBridge()`. When the cap is hit the turn returns its partial text plus a truncation notice (`emitToolRoundLimitNotice`) instead of throwing.
...
`electron/agent/codex.ts` runs turns against **`@openai/codex-sdk`**. Unlike every other provider, this SDK is **not an HTTP client** — it spawns the **Codex CLI binary** and runs Codex as a self-driving coding agent that executes its own tools (shell, file edits, MCP) inside its own sandbox. This inverts the usual control model, so several conventions differ:

- **Two transports (per-turn switch).** Interactive turns run over the long-running **`codex app-server`** JSON-RPC transport (`electron/agent/codex-app-server.ts`) so Codex's `on-request` approvals bridge to AIchemist's approval UI; **noTools / nonInteractive turns** (read-only or unattended) keep the one-shot **`codex exec`** SDK transport. `codexProvider.run()` dispatches: `useAppServer = !noTools && !nonInteractive`. If the app-server can't come up (spawn/initialize/thread-start throws — including a synchronous `spawn` failure, since construction is inside the try), `runViaAppServer` returns `{ fallback: true }` and the turn re-runs on exec (degraded to `on-failure`), so interactive turns never hard-fail on the experimental transport. A failure *after* streaming begins is a real error and propagates. Both paths share the prologue (emitter, recorder, item sink, model, input, thread persistence) and the `TurnRunContext`.
- **Approval gate.** The exec transport can't do interactive callbacks, so Codex there just self-executes (`resolveSandboxPolicy()` maps the flags onto `sandboxMode`/`approvalPolicy`: `noTools` → read-only + never; `nonInteractive` → workspace-write + never; exec-fallback interactive → workspace-write + on-failure). The **app-server** transport uses `workspaceWrite` + **`on-request`** and bridges each `item/commandExecution/requestApproval` / `item/permissions/requestApproval` through `resolveCodexApproval()` (`electron/agent/codex-approval-bridge.ts`) onto the **existing** gate (`requiresApproval` → `SESSION_APPROVAL_REQUIRED` dialog → `resolveApproval`) — no new UI. Command approvals reply `{ decision: "approved" | "denied" }` (Codex `ReviewDecision` wire values — the one place to change if the experimental protocol shifts); permission (filesystem-write) requests reply with the granted subset `{ scope, permissions }` (v1 grants the full requested set on allow, none on deny). Already-trusted calls (session/project allowlist) auto-allow with no prompt.
- **Thread model:** exec uses `Codex.startThread(opts)` / `resumeThread(id, opts)` then `thread.runStreamed(input)`; the app-server uses `thread/start` / `thread/resume` then `turn/start` (its per-turn process resumes the persisted thread each turn). Both persist the thread id via `providerSessionStore` under the `codex` slice (`{ threadId }`) — one shared id, so a thread started on one transport resumes on the other. Model / sandbox / approval / `workingDirectory` are set at **thread creation**, not per-turn.
- **No system-prompt param.** Codex uses its own config, so the composed skills/agent-body/memory context (`buildSystemPreamble`) is **prepended to the turn input** rather than passed as `instructions`.
- **Event mapping:** `thread.started` → persist thread id; `item.completed` `agent_message` → `emitter.delta()` (whole-message chunks, not token deltas); `reasoning` → recorder; `command_execution` / `file_change` / `mcp_tool_call` / `web_search` items → `emitter.toolCall`/`toolResult` + recorder (reflected, not gated); `turn.completed` → usage (`cached_input_tokens` → `cache_read_input_tokens`); `turn.failed` / `error` → throw. The item→emitter/transcript reflection is **transport-agnostic**: `electron/agent/codex-item-mapper.ts` owns `createCodexItemSink()` (started/completed dedup + Changes-panel filtering) over a `NormalizedCodexItem` union; the exec path adapts SDK `ThreadItem`s via `fromSdkThreadItem()` in `codex.ts`, and the app-server path adapts its camelCase `item/*` payloads via `fromAppServerItem()` (in the mapper module — pure `unknown`-parsing), so both transports render identically. The app-server run loop maps `item.started`/`item.completed` through the sink and `turn.completed` usage via `emitAppServerUsage()` (best-effort field names). Test seam: `_setAppServerConnectorForTests` injects a fake JSON-RPC peer (no binary spawn) so the transport switch + approval bridge are unit-tested.
- **Managed MCP:** AIchemist-managed servers are injected via `CodexOptions.config` (`{ mcp_servers: toCodexMcpServers(loadManagedMcpServers({ excludeNames })) }`, honoring the per-session disable set), so Codex calls them and emits `mcp_tool_call` items. Because the SDK passes `--config` on every `codex exec` spawn, toggling a server takes effect next turn with NO resume-invalidation fingerprint (unlike Copilot). The Codex client is therefore built per-turn (not a cached singleton) with that config; `noTools` turns inject none. Test seam: `_setCodexFactoryForTests` captures the resolved `CodexOptions`.
...
- **Runtime binary:** the exec path lets the SDK resolve the `codex` binary internally (bundled `@openai/codex-<platform>` package, or `CODEX_CLI_PATH` → `codexPathOverride`). The **app-server** path spawns the binary itself, so `electron/agent/codex-binary.ts` (`resolveCodexBinary()`) mirrors the SDK's private resolver — platform/arch → target triple → `<platform-pkg>/vendor/<triple>/bin/codex` (+ `codex-path` dirs prepended to the child PATH via `spawnAppServerConnector`'s `pathDirs`). `CODEX_CLI_PATH` overrides; an unresolvable binary returns null → the connector falls back to PATH `"codex"` → a missing binary fails startup → the turn falls back to exec. Same node_modules layout the exec path already needs, so no new packaging requirement (the binary must still ship with the app). Auth uses the OpenAI API key (`getApiKey("openai")`); `OPENAI_BASE_URL` → `baseUrl`.
...
| Loader / adapters | `electron/mcp/managed.ts` — `loadManagedMcpServers()`, `toClaudeMcpServers()`, `toCopilotMcpServers()`, `fingerprintManaged()` |
...
| Codex injection | `electron/agent/codex.ts` builds `CodexOptions.config = { mcp_servers: toCodexMcpServers(...) }` per turn. Codex re-reads `--config` on every `codex exec` spawn, so no resume-invalidation fingerprint is needed. |
| Panel | `McpServersPanel` shows a violet "AIchemist" badge for `source === "aichemist"`. It keeps the per-session disable toggle + refresh and deep-links into the Settings hub (`openSettings({ scope: "app", id: "mcp" })`) for configuration. |
| Hub section | `McpServersSection` (Settings → MCP Servers) is the single MCP **config** editor (the old `McpConfigEditorDialog` was retired). Scope tabs (AIchemist default + Claude/Copilot scopes), form/JSON modes, add/edit/delete via `mcpReadConfig` / `mcpWriteConfig` / `mcpDeleteServer`, and per-row live health (connected / tool count / error) for the AIchemist scope via `mcpProbeManaged` with a force-refresh. |
...
Full-stack work surfaces the agent and the user both operate on — a kanban board, a SQLite browser, a game — shared by the agent (via tools) and the user (via a UI in the right panel). Design doc: `notes/plans/2026-09-25-canvas-design.md`. Phase 1 (runtime spine + a built-in) is done; discovery tiers/trust (project canvases), `ctx.agent.send`, and `create-canvas` authoring are later phases.

| Concept | Detail |
|---|---|
| **Canvas definition** | A folder: `canvas.json` (manifest) + `server.mjs` (SDK-based server module) + `ui/` (iframe UI). Resolved by `electron/canvas/definitions.ts` across tiers — currently **global** (`~/.aichemist/canvases/`, user-provided, trusted) and **built-in** (`electron/canvas/builtin/`, shipped with the app, trusted as app code); a name in both resolves to the global one. Project-tier discovery (`<projectPath>/.agents/canvases/`) is untrusted-by-default and deferred to a later issue's trust prompt. |
| **Canvas instance** | A definition bound to a project with its own persisted JSON state (`electron/canvas/store.ts` — `canvases` / `session_canvases` / `canvas_trust` tables). Attaching an instance to a session (`setCanvasAttached`) is what exposes its tools to that session's agent. |
| **Canvas host** | The running process for one instance — an Electron `utilityProcess.fork()` (`electron/canvas/host-manager.ts`'s `CanvasHostManager`), isolated from main so a crash/hang in canvas code can't affect the app. Talks to main only over its own `MessagePort`, validated against `electron/canvas/host-protocol.ts`. |
| **Server SDK** | `@aichemist/canvas` (`electron/canvas/host/sdk.ts`) — `defineCanvas({ initialState, tools, onUiMessage })` + a re-exported `z` (zod). Resolved with **no install step**: a Node ESM loader hook (`host/loader-hook.ts`, registered via `module.register()` in `host/entry.ts`) maps the bare specifier to a real-ESM build (`scripts/build-canvas-sdk-esm.mjs` → `dist/main/host/sdk.mjs` — a CJS build can't satisfy the named `z` re-export via CJS/ESM interop, see that script's header). |

**Agent tools — one loopback MCP endpoint for every provider.** `electron/canvas/mcp-endpoint.ts`'s `CanvasMcpEndpoint` runs a single streamable-HTTP MCP server on `127.0.0.1` (random port, per-launch bearer token). Each attached instance is reachable at `/canvas/<canvasId>/session/<sessionId>/mcp` and injected as a managed MCP server (`canvasMcpServersForSession()`, merged wherever a provider calls `loadManagedMcpServers()`) — so Claude, Copilot, Codex, Ollama, and OpenAI-compatible all get canvas tools through the same HTTP-entry adapters `electron/mcp/managed.ts` already has, no per-provider code. The endpoint itself enforces approval (`requiresApproval()`/`requestApproval()`, since e.g. Claude's `PreToolUse` hook passes `mcp__*` tools through unexamined) and scoping (bad token, or a canvas not attached to that session, is rejected before touching the host).

**UI — sandboxed iframe.** `aichemist-canvas://<canvasId>/` (`electron/canvas/protocol.ts`) serves a definition's `ui/` folder with `sandbox="allow-scripts"` and **no** `allow-same-origin` (opaque origin — no access to the host DOM/storage/`window.electronAPI`), plus a CSP with `connect-src 'none'` (the UI has no network; anything network-bound goes through the trusted server). The bridge (`src/components/session/CanvasFrame.tsx` ⇄ `aichemist-canvas-client.js`, served at a fixed path by the protocol) gives a canvas's UI `canvas.onState(fn)` / `canvas.onMessage(fn)` / `canvas.send(msg)` / `canvas.getState()`, plus the `ready` handshake and light/dark theme sync. `CanvasPanel` (the panel's "Canvas" tab) drives the instance picker, attach toggle, host status, and a debug log drawer; `useCanvasStore` mirrors `CANVAS_EVENT` pushes.

**Built-in: kanban** (`electron/canvas/builtin/kanban/`) — the worked example that proves the SDK end to end, dogfooding the exact path a user-authored canvas will use. `server.mjs` is two lines (`import { defineCanvas, z } from "@aichemist/canvas"` + `createKanbanDefinition(z)`); the actual reducer logic lives in `board.mjs` (pure, SDK-free — `createBoard`/`addCard`/`moveCard`/`updateCard`/`removeCard`/`findCard`) and `definition.mjs` (the shared `CanvasServerDefinition` factory), so `onUiMessage` and the tools apply the *same* state-transform functions — one reducer, not two copies of "how a card moves". Tools: `get_board`/`add_card`/`move_card` are `approval: "none"`; `update_card`/`remove_card` (mutate/delete existing data) default to `"ask"`. `ui/` is plain HTML/CSS/JS (no build step, matching the protocol's "unbundled files under `ui/`" contract) with native HTML5 drag-and-drop.

- **Why `definition.mjs` is split out of `server.mjs`:** the SDK's `@aichemist/canvas` bare specifier only resolves inside a real canvas host process (the loader hook needs a real ESM loader thread — same reason `loader-hook.ts` itself isn't unit-tested). `definition.mjs` takes `z` as a parameter instead of importing it from the SDK, so tests can build the exact same `CanvasServerDefinition` by importing real `zod` directly, with zero logic duplicated between what ships and what's tested.
- **Registering a new built-in:** add its name to `BUILTIN_DEFINITION_NAMES` in `electron/canvas/definitions.ts`, and ship its folder under `electron/canvas/builtin/<name>/`. Built-in canvas assets are plain, unbundled files (not compiled by electron-vite) — `electron-builder.yml`'s `files` list includes `electron/canvas/builtin/**` alongside `dist/main/**` so they ship at the same relative path inside the packaged app that `definitions.ts` resolves them from (computed relative to `__dirname`, which is `dist/main` at runtime — two directories below the app root in both dev and a packaged build, the same depth `electron/canvas` sits at under vitest).
- **`CANVAS_LIST_DEFINITIONS`** (`electron/ipc/canvas-handlers.ts`) currently lists only the built-in tier (`listBuiltinCanvasDefinitions()`, reading each one's real `canvas.json`) — what lets "New canvas…" in the panel offer kanban from a dropdown before the full project/global discovery tiers land.

**Testing seams:**

| Seam | Where | Purpose |
|---|---|---|
| `_setCanvasesRootForTests(dir \| null)` | `electron/canvas/definitions.ts` | Override the global tier's root directory. |
| `_setBuiltinCanvasesRootForTests(dir \| null)` | `electron/canvas/definitions.ts` | Override the built-in tier's root directory (real kanban is left alone in most tests — only used to test tier-priority/fail-safety against a fake root). |
| `CanvasHostManagerOptions.spawn` | `electron/canvas/host-manager.ts` | Inject a fake `CanvasHostProcessFactory` instead of a real `utilityProcess.fork()` — `vi.mock("electron", () => ({ utilityProcess: { fork: vi.fn() } }))` first (hoisted above imports), then a fake process factory that runs a *real* `createCanvasHostRuntime()` against a real `CanvasServerDefinition`, so tests exercise the manager ⇄ host round trip through the actual host message loop. `electron/canvas/builtin/kanban/kanban-e2e.test.ts` does exactly this against the shipped kanban definition (keyed by its real, resolved `server.mjs` path) — an agent tool call over the real HTTP `CanvasMcpEndpoint` all the way through the real host runtime, proving the loop `CanvasHostManagerOptions.hooks.onStateChanged` (the same hook `main.ts` wires to push `CANVAS_EVENT`) fires immediately, and that a UI message (`hostManager.sendUiMessage`) is visible on the next `get_board`. |
| `_setCodexFactoryForTests`-style pattern | n/a (documented for reference) | The same "fake the process boundary, run the real logic behind it" shape other providers already use — canvases follow it rather than inventing a new one. |
...
Workflows are saved, repeatable agent tasks (see `notes/plans/2026-06-21-workflow-scheduling-design.md`). CRUD lives in `electron/workflows.ts` (tables `workflows` / `workflow_runs`); the turn-execution core is `electron/agent/workflow-scheduler.ts`.
...
- **`WorkflowScheduler` class** (in `workflow-scheduler.ts`) is the **trigger manager** — it owns both the cron jobs and the file-watch watchers (see "Workflow triggers" below). Constructed in `main.ts` whenReady with `{ db, activeTurns, getMainWindow }`, (optionally) hooks, and an optional `{ fileWatchDebounceMs }` (tests use a short window); `start()` arms every enabled workflow that declares a `cron` **or** a `watch_path` after `registerAllHandlers()`, `stopAll()` runs on `before-quit`. `rearm(id)` stops the old triggers and re-arms from the current DB row (a disabled / trigger-cleared row ends up unarmed); `cancel(id)` stops a workflow's triggers; `delete(id)` cancels then removes rows; `runNow(id, trigger?)` runs with the scheduler's notify+push hooks. **Forward-only** — `croner` fires from now and file events only fire for changes made while watching; neither replays anything missed while the app was closed. A failing run never disarms its triggers. Default hooks push `WORKFLOW_RUN_UPDATED` to an open renderer and fire an OS `Notification` on terminal states (both fail-safe — a missing window/notification never breaks a run). `onJobsChanged(listener)` registers a single fail-safe listener fired whenever the armed set may have changed (`start` / `rearm` / `cancel`); `main.ts` uses it to keep the tray in sync, and `armedCount` (a workflow with both triggers counts once) is the live count of enabled scheduled workflows.
- **Workflow triggers** — a workflow may declare a `cron`, a `watch_path`, **both, or neither** (manual-only). `cron` arms a `croner` job; `watch_path` arms an `fs.watch(path, { recursive: true })` watcher whose change events are coalesced through a per-workflow debounce (`FILE_WATCH_DEBOUNCE_MS`, default 500 ms) before firing a run with `trigger: "file"`. File-triggered runs go through the same headless `runWorkflow` path as cron (so the same autonomy / overlap / notification plumbing applies). Arming is fail-safe: an unwatchable path (missing, permission denied, recursive-watch unsupported) is logged and skipped, never aborting the rest, and a delayed watcher `error` drops just that watcher. Because `armedCount` includes file-watch workflows, the tray / survive-window-close behavior covers them too.
- **Tray / survive window close:** `electron/tray.ts` (`TrayController`) owns an optional system-tray icon, present **only while `scheduler.armedCount > 0`** (≥1 enabled scheduled workflow). `refresh()` is idempotent — it creates the tray when the count rises above zero, tears it down when it returns to zero, and otherwise refreshes the tooltip + menu ("Open AIchemist" / count label / "Quit AIchemist"). The left-click → reopen handler is attached off macOS only (macOS shows the context menu on click by convention). `main.ts` wires `scheduler.onJobsChanged(() => tray.refresh())`. The non-darwin `window-all-closed` handler no longer quits unconditionally: it keeps the app alive (tray-only, scheduler still firing) when `armedCount > 0` **and `tray.isActive()`** — gating on the tray actually existing means a failed tray creation falls back to quitting rather than stranding a windowless process with no reopen/quit handle; otherwise it quits as before. An `isQuitting` flag (set by the tray "Quit" action and `before-quit`) forces a real quit through. The tray icon is an embedded base64 PNG (no main-process asset pipeline). `onJobsChanged` fires **once per high-level scheduler op** (the low-level `stopJob` is silent; `start`/`rearm`/`cancel` emit a single notification after the armed set settles) so the tray never sees a transient count drop mid-`rearm`. Firing while the app is *fully closed* (OS-level launchd/Task Scheduler) remains out of scope.
- **IPC:** `WORKFLOW_LIST` (list workflows, optionally by project), `WORKFLOW_UPSERT` (create when no id / patch when id resolves — re-arms the scheduler), `WORKFLOW_RUN_NOW` (`trigger: "manual"`), `WORKFLOW_DELETE` (cancels the job, cascades runs), `WORKFLOW_LIST_RUNS` (run history), and the `WORKFLOW_RUN_UPDATED` push event. Handlers in `electron/ipc/workflow-handlers.ts` (registered with the scheduler instance). `WORKFLOW_UPSERT` has a zod validator in `validators.ts` whose `cron` field is checked with `isValidCron()` (croner) so an unparseable schedule is rejected at the boundary; its `watchPath` is only rejected when whitespace-only (existence isn't required at save time — the path may be created later, and the watcher arms fail-safe).
...
### Workflows — renderer view

The **Workflows** view (`src/components/workflows/`) is a full-screen surface toggled by `workflowsOpen` in `useProjectStore` (mutually exclusive with `settingsOpen`), opened from the `CalendarClock` button in `ProjectSidebar` and rendered by `AppShell`.

- **`WorkflowsView`** — left nav lists all workflows (enabled dot + trigger icons + project name): a `Clock` for cron, an `Eye` for a file `watch_path`, a `Hand` for manual-only (icons combine when a workflow has both). The right panel shows either the editor or a read-only detail (which surfaces the `watch_path` in a "File trigger" meta) with the run-history panel. Loads workflows via `ipc.workflowList()` and subscribes to `WORKFLOW_RUN_UPDATED` to live-update run history.
- **`WorkflowEditor`** — create/edit form (name, prompt, project, provider/model, agent, skills, cron, watch path, session strategy, autonomy, enabled). The cron field shows a live human-readable "next run" preview via `previewCron()` in `src/lib/cron.ts` (renderer-safe `croner`, mirrors `electron/cron.ts` validation); an invalid expression disables save. The watch-path field has a "Browse…" button backed by `ipc.openFolderDialog()`. `autonomy: "autonomous"` shows a prominent `role="alert"` warning — it is the app acting on the filesystem/shell with no human in the loop.
- **`WorkflowRunHistory`** — status/trigger/timing rows with a "Run now" button and a "Session" link into the run's session. Run-now resolves the new run row, then re-fetches history.
- **`useWorkflowStore`** (Zustand, not persisted — SQLite is the source of truth) caches `workflows` + `runsByWorkflow`; `applyRunUpdate(run)` merges a pushed run into an already-loaded history list (it no-ops for an un-cached workflow, which re-fetches on open).
