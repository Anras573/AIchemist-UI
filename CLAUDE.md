...
- **Extended thinking / reasoning:** gated on model capability — `modelSupportsThinking()` reads `ollama.show()`'s `capabilities` (cached per model id; cleared by `_resetOllamaClientForTests`) and only then passes `think: true` to `chat()`, because requesting thinking from a non-reasoning model errors. In the stream, `message.thinking` deltas map to `emitter.thinkingDelta()` (+ `recorder.reasoning()`); there is no explicit end marker, so the reasoning block is closed (`thinkingDone()`) when the first `content`/`tool_call` arrives or at end of stream. The probe is fail-safe (any error → no thinking, never breaks the turn), skipped for `noTools` turns, and `think` is never requested for delegated sub-agents (their output is internal). This lights up the same `Reasoning` component the other providers use.
