...
- **IPC:** `WORKFLOW_UPSERT` (create when no id / patch when id resolves) and `WORKFLOW_RUN_NOW` (`trigger: "manual"`), handlers in `electron/ipc/workflow-handlers.ts`. `WORKFLOW_UPSERT` has a zod validator in `validators.ts` whose `cron` field is checked with `isValidCron()` (croner) so an unparseable schedule is rejected at the boundary.
- **Cron** uses **`croner`** (zero-dep, TS-native, DST-aware). `validateCron()` / `isValidCron()` parse with `{ paused: true }` so validating never arms a timer. **Arming jobs on boot/edit is a later phase** — these handlers only persist and run on demand.
