...
- **`WorkflowScheduler` class** (in `workflow-scheduler.ts`) owns the cron jobs. Constructed in `main.ts` whenReady with `{ db, activeTurns, getMainWindow }` and (optionally) hooks; `start()` arms every enabled workflow with a `cron` after `registerAllHandlers()`, `stopAll()` runs on `before-quit`. `rearm(id)` stops the old job and re-arms from the current DB row (a disabled / cron-cleared row ends up with no job); `cancel(id)` stops a job; `delete(id)` cancels then removes rows; `runNow(id, trigger?)` runs with the scheduler's notify+push hooks. **Forward-only** — `croner` fires from now; missed occurrences while the app was closed are not replayed. A failing run never disarms its job. Default hooks push `WORKFLOW_RUN_UPDATED` to an open renderer and fire an OS `Notification` on terminal states (both fail-safe — a missing window/notification never breaks a run). `onJobsChanged(listener)` registers a single fail-safe listener fired whenever the armed-job set may have changed (`start` / `rearm` / `cancel`); `main.ts` uses it to keep the tray in sync, and `armedCount` is the live count of enabled scheduled workflows.
...
- **`WorkflowsView`** — left nav lists all workflows (enabled dot + cron/manual icon + project name); the right panel shows either the editor or a read-only detail with the run-history panel. Loads workflows via `ipc.workflowList()` and subscribes to `WORKFLOW_RUN_UPDATED` to live-update run history.
- **`WorkflowEditor`** — create/edit form (name, prompt, project, provider/model, agent, skills, cron, session strategy, autonomy, enabled). The cron field shows a live human-readable "next run" preview via `previewCron()` in `src/lib/cron.ts` (renderer-safe `croner`, mirrors `electron/cron.ts` validation); an invalid expression disables save. `autonomy: "autonomous"` shows a prominent `role="alert"` warning — it is the app acting on the filesystem/shell with no human in the loop.
